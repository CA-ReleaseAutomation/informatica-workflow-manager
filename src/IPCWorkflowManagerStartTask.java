 ...
 */
@ActionDescriptor(
        name = "IPC - Workflow Manager - Start Task",
		description = "Start an Informatica workflow",
        category="PowerCenter.Workflow Manager" )
    public class IPCWorkflowManagerStartTask extends Action {
    private static final long serialVersionUID = 1L;
	private static final Logger log = Logger.getLogger(IPCWorkflowManagerStartTask.class);
	...
	@ParameterDescriptor(
        ...
        defaultValueAsString = ""
            ...
            private String taskinstancepath7 = "C:\\\\Informatica\\\\9.5.1\\\\clients\\\\PowerCenterClient\\\\CommandLineUtilities\\\\PC\\\\server\\\\bin";
	...
	@ParameterDescriptor(
        ...
        defaultValueAsString = ""
            ...
            private String parameterfile8;
            

	@ParameterDescriptor(
		name = "Workflow Run Instance",
		description = "Workflow Run Instance Description",
		out = false,
		in = true,
		nullable = true, // parameter not required or default value set
		order = 90,
        defaultValueAsString = ""
            ...
            private String workflowruninstance9;
	...
	private String runinstancename4;
    ...
    public void setfolder6(String folder6) {
    	this.folder6 = folder6;
	        ...
	        public String gettaskinstancepath7() {
        return taskinstancepath7;
    }
    
    public void settaskinstancepath7(String taskinstancepath7) {
    	this.taskinstancepath7 = taskinstancepath7;
    }
	        public String getparameterfile8() {
        return parameterfile8;
    }
    
    public void setparameterfile8(String parameterfile8) {
    	this.parameterfile8 = parameterfile8;
    }
	        public String getworkflowruninstance9() {
        return workflowruninstance9;
    }
    
    public void setworkflowruninstance9(String workflowruninstance9) {
    	this.workflowruninstance9 = workflowruninstance9;
    }
	    // Getters and Setters
    ...
    public void setexitcode3(Integer exitcode3) {
    	this.exitcode3 = exitcode3;
	        ...
	        public String getruninstancename4() {
        return runinstancename4;
    }
    
    public void setruninstancename4(String runinstancename4) {
    	this.runinstancename4 = runinstancename4;
    }
	        public String getrunid5() {
        return runid5;
    }
    
    public void setrunid5(String runid5) {
    	this.runid5 = runid5;
    }
    ...
    private void populateOutputParameters(ExecutionResult execResult) {
        ...
        if (StringUtils.isNotEmpty(execResult.getStandardOutput())) {
            output = execResult.getStandardOutput();
                            try {
                Pattern pattern = Pattern.compile("(?<=run id \\[)[0-9]*");
                Matcher matcher = pattern.matcher(output);
                String stringMatch = null;
                if (matcher.find()) {
                    stringMatch = matcher.group();
                } else{
                    log.info("Cound not find a match for " + output + " with (?<=run id \\[)[0-9]*");
                }

                runid5 = String.valueOf(stringMatch);
                log.info("Got output " + runid5);
            } catch (PatternSyntaxException e) {
                log.error("Caught exception during populating output parameter: " + "run id" + ". Bad pattern", e);
            } catch (Exception e) {
                log.error("Caught exception during populating output parameter: " + "run id", e);
            }
                }
	...
	private ActionResult getActionResult(com.ca.nolio.rdk.dto.CLIAction action) {
 	...
 	private List<InputParam> getInput() {
			...
			inputParameters.add(p);		
					p = new InputParam();
			p.setName("Task Instance Path");
			 p.setDescription("Workflow Description");
             p.setDefaultValue("2147483647");
             p.setPrefix("");
			//p.setValue("");
			p.setVariableName("taskinstancepath");
			p.setType(VarType.String);

									    			 				p.setValue( taskinstancepath7 );
			 								
			inputParameters.add(p);		
					p = new InputParam();
			p.setName("Parameter File");
			 p.setDescription("Parameter File Description");
             p.setDefaultValue("");
             p.setPrefix("–paramfile");
			//p.setValue("");
			p.setVariableName("parameterfile");
			p.setType(VarType.String);

									    			 				p.setValue( parameterfile8 );
			 								
			inputParameters.add(p);		
					p = new InputParam();
			p.setName("Workflow Run Instance");
			 p.setDescription("Workflow Run Instance Description");
             p.setDefaultValue("");
             p.setPrefix("-rn");
			//p.setValue("");
			p.setVariableName("workflowruninstance");
			p.setType(VarType.String);

									    			 				p.setValue( workflowruninstance9 );
	...
	private List<OutputParam> getOutput() {
			...
			outputParameters.add(p);					
					p = new OutputParam();
			p.setName("run instance name");
			            			p.setVariableName("runinstancename");
			p.setType(VarType.String);
			
							p.setFilterType(FilterType.REGEX);
						
									    			 				p.setFilterValue( runinstancename4 );
	...
	private List<CliCommand> getCliCommands(){
		...
		Script script = null;
					...
					cliCmd.setCommand("pmcmd starttask -service ${Service} -d ${Domain} -u ${User} -p ${Password} -f ${Folder}  ${Parameter File} ${Task Instance Path}");											
...
private com.ca.nolio.rdk.dto.CLIAction getCLIAction() {
	...
	action.setName("IPC - Workflow Manager - Start Task");
