 ...
 */
@ActionDescriptor(
        name = "IPC - Workflow Manager - Abort Workflow",
		description = "Start an Informatica workflow",
        category="PowerCenter.Workflow Manager" )
    public class IPCWorkflowManagerAbortWorkflow extends Action {
    private static final long serialVersionUID = 1L;
	private static final Logger log = Logger.getLogger(IPCWorkflowManagerAbortWorkflow.class);
	...
	private ActionResult getActionResult(com.ca.nolio.rdk.dto.CLIAction action) {
 	...
 	private List<InputParam> getInput() {
			...
			inputParameters.add(p);		
					p = new InputParam();
			p.setName("Run ID");
			 p.setDescription("Run ID Description");
             p.setDefaultValue("");
             p.setPrefix("-wfrunid");
			//p.setValue("");
	...
	private List<CliCommand> getCliCommands(){
		...
		Script script = null;
					...
					cliCmd.setCommand("pmcmd abortworkflow -service ${Service} -d ${Domain} -u ${User} -p ${Password} -f ${Folder} ${Workflow} ${Parameter File}");											
...
private com.ca.nolio.rdk.dto.CLIAction getCLIAction() {
	...
	action.setName("IPC - Workflow Manager - Abort Workflow");
